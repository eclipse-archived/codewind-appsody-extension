...
if [ "$IN_K8" == "true" ]; then
	...
	export APPSODY_MOUNT_HOME=appsody-$projectName-cwdeps
...
# Remove the application
	...
	# else
		...
		if [ "$IN_K8" != "true" ]; then
			...
			$IMAGE_COMMAND volume ls -q -f name=appsody-$projectName- | xargs $IMAGE_COMMAND volume rm 2> /dev/null || true
		fi
	# fi
